...
class SupervisorController extends BaseController
{
    ...
    public function getCapaianBawahan(){
        ...
        $rancangan_tugas = model('rancangan_tugas');
        $tugas = model('tugas');
        $data['rancangan_tugas'] = $rancangan_tugas->where('id_jabatan', $id_jabatan)->join('rancangan_per_tahun as r', 'rancangan_tugas.id_rancangan_tugas = r.id_rancangan_tugas')->where('r.tahun', $tahun)->findAll();
    ...
    public function daftarPenilaian($no_induk)
    {
        ...
        $status_pk = $pk->where('status_pk', 1)->first();
        // dd($cek_pk);

        if ($status_pk == null) {
            $data['info'] = 'Tidak sedang dilakukan penilaian kinerja ';
            $data['pertanyaan'] = null;
            $data['hasil'] = null;
        }else if ($status_pk != null && $cek_pk == null) {
            $data['info'] = 'Anda belum melakukan pemberian nilai kinerja, Silahkan melakukan penilaian kinerja !! ';
    ...
    public function savePertanyaanPenilaian($no_induk)
    {
        ...
        return redirect()->to(base_url().'/supervisor/daftarPenilaian/' . $no_induk);
    }
